  ...
  s3.listObjects({ Delimiter: "/" }, function (err, data) {
    if (err) {
      ...
      var message = albums.length
        ...
        : "<p style=\"text-align: center;\">강의 시작 전 수강생들의 이미지를 업로드 합니다.";
      var htmlTemplate = [
        message,
        "<ul>",
        getHtml(albums),
        "</ul>",
        '<form>',
        "<input type='text' class='input-field' id='albumName' placeholder=\"과목 이름을 입력해주세요.\"required autofocus></input>",
        "<button class='submit' type='button' button onclick=\"createAlbum(albumName.value)\" >",
        "과목 생성",
        "</button>",
...
function autoviewAlbum(albumName) {
  ...
  s3.listObjects({ Prefix: albumPhotosKey }, function (err, data) {
    ...
    var htmlTemplate = [
      ...
      getHtml(photos),
      "</ul>",
      '<input multiple id="photoupload" type="file" accept="image/*" name="filename[]" style="botton">',
...
function addPhoto(albumName) {
  ...
  if (!files.length) {
    return submitErrMSG.value = "에러: 손상된 이미지"
  }
  else {
